...
function onesocial_setup() {
	...
	// Declare BuddyPress Legacy template pack support
...
// Load our own adminbar (Toolbar) styles.
	...
	$buddyboss_js_vars = array(
		...
		'excluded_inputs' => onesocial_get_option( 'boss_excluded_inputs' ),
		'myprofile_menu'  => get_term( get_nav_menu_locations()['header-my-account'], 'nav_menu' )->slug
	);
	...
	// Localize nonce of registration and login process
	if ( !is_user_logged_in() ) {
        $transport_array['loginNonce']      = wp_create_nonce('ajax-login-security');
        $transport_array['registerNonce']   = wp_create_nonce('ajax-register-security');
    }
...
function buddyboss_add_handle() {
	echo '<span class="bbp-user-nicename"><span class="handle-sign">@</span>' .  get_the_author_meta( 'nicename', bbp_get_reply_author_id( bbp_get_reply_id() ) ) . '</span>';
}
...
if ( !function_exists( 'buddyboss_get_options_nav' ) ) :
	...
	function buddyboss_get_options_nav( $parent_slug = '' ) {
		...
		$sentbox = (int) $wpdb->get_var( $wpdb->prepare("SELECT COUNT(m.thread_id) FROM {$bp->messages->table_name_messages} m INNER JOIN (SELECT DISTINCT thread_id FROM {$bp->messages->table_name_recipients} WHERE is_deleted = 0 AND user_id = %d) r ON r.thread_id = m.thread_id WHERE m.sender_id = %d", $user_id, $user_id ) );
...
// Should we show the time also?
	if ( empty( $just_date ) ) {
		...
		$date = sprintf( __( '%1$s at %2$s', 'onesocial' ), $date, $time );
	}
					...
					<?php foreach ( (array) $group->admins as $admin ) { ?>
						<li>
							<a class="group-admin-container" href="<?php echo bp_core_get_user_domain( $admin->user_id, $admin->user_nicename, $admin->user_login ) ?>">
								<?php echo bp_core_fetch_avatar( array( 'item_id' => $admin->user_id, 'email' => $admin->user_email, 'alt' => sprintf( __( 'Profile picture of %s', 'onesocial' ), bp_core_get_user_displayname( $admin->user_id ) ) ) ) ?>
			...
			if ( !empty( $group->mods ) ) :
				...
				<ul id="group-mods">
					...
					<?php foreach ( (array) $group->mods as $mod ) { ?>
						...
						<li>
							<a href="<?php echo bp_core_get_user_domain( $mod->user_id, $mod->user_nicename, $mod->user_login ) ?>"><?php echo bp_core_fetch_avatar( array( 'item_id' => $mod->user_id, 'email' => $mod->user_email, 'alt' => sprintf( __( 'Profile picture of %s', 'onesocial' ), bp_core_get_user_displayname( $mod->user_id ) ) ) ) ?></a>
...
/**
 * Determine whether to display admin bar or not
 * @return mixed|void
 */
function onesocial_show_adminbar() {
	$show = false;

	if ( ! is_admin()
		&& current_user_can( 'manage_options' )
		&& onesocial_get_option( 'boss_adminbar' )
		&& ! is_phone() ) {
	// Other user type cannot see top bar.
		$show = true;
	}
