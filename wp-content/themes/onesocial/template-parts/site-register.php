			...
			<div class="row">
				...
				<div class="<?php echo $class; ?> with-email">
                    <form method="POST" action="<?php echo admin_url( 'admin-ajax.php' );?>" id="frm_siteRegisterBox">
						...
						<p class="email-wrap">
							<input type="email" id="register_email" name="register_email" placeholder="<?php _e( 'Email', 'onesocial' ) ?>" class="input" />
						</p>
						...
						<?php if ( bp_get_blog_signup_allowed() ) : ?>
							...
							<div class="register-section" id="blog-details-section">
								...
								<h2><?php _e( 'Blog Details', 'onesocial' ); ?></h2>

								<p><label for="signup_with_blog"><input type="checkbox" name="signup_with_blog" id="signup_with_blog" value="1"<?php if ( (int) bp_get_signup_with_blog_value() ) : ?> checked="checked"<?php endif; ?> /> <?php _e( 'Yes, I\'d like to create a new site', 'onesocial' ); ?></label></p>

								<div id="blog-details"<?php if ( (int) bp_get_signup_with_blog_value() ) : ?>class="show"<?php endif; ?>>

									<label for="signup_blog_url"><?php _e( 'Blog URL', 'onesocial' ); ?> <?php _e( '(required)', 'onesocial' ); ?></label>
									<?php
									...
									<label for="signup_blog_title"><?php _e( 'Site Title', 'onesocial' ); ?> <?php _e( '(required)', 'onesocial' ); ?></label>
									<?php
									...
									<fieldset class="register-site">
										<legend class="label"><?php _e( 'Privacy: I would like my site to appear in search engines, and in public listings around this network.', 'onesocial' ); ?></legend>
										<?php
										...
										<label for="signup_blog_privacy_public"><input type="radio" name="signup_blog_privacy" id="signup_blog_privacy_public" value="public"<?php if ( 'public' == bp_get_signup_blog_privacy_value() || !bp_get_signup_blog_privacy_value() ) : ?> checked="checked"<?php endif; ?> /> <?php _e( 'Yes', 'onesocial' ); ?></label>
										<label for="signup_blog_privacy_private"><input type="radio" name="signup_blog_privacy" id="signup_blog_privacy_private" value="private"<?php if ( 'private' == bp_get_signup_blog_privacy_value() ) : ?> checked="checked"<?php endif; ?> /> <?php _e( 'No', 'onesocial' ); ?></label>
									</fieldset>
