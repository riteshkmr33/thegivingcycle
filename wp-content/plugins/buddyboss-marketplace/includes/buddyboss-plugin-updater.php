...
add_filter( 'bboss_licensed_packages', 'socmp_register_licensed_package' );
function socmp_register_licensed_package( $packages=array() ){
    $this_package = array(
        'id'        => 'socmp',
        'name'      => __( 'Social MarketPlace', 'buddyboss-marketplace' ),
        'products'  => array(
    ...
    $packages['socmp'] = $this_package;
    return $packages;
}

add_filter( 'bboss_updatable_products', 'socmp_register_updatable_product' );
function socmp_register_updatable_product( $products ){
    ...
    $products['SOCMP'] = array(
        ...
        'path'          => 'buddyboss-marketplace/buddyboss-marketplace.php',
        'id'            => 195,
        'software_ids'  => array( 'SOCMP_UPGRADE', 'SOCMP_1S', 'SOCMP_5S', 'SOCMP_20S' ),
        'type'          => 'plugin',
    );
...
if( !function_exists( 'bboss_notice_install_updater' ) ) {
    ...
    function bboss_notice_install_updater(){
        ...
        $notice_install_buddyboss_updater = sprintf( __( "To receive updates for BuddyBoss products, please install and activate the plugin %s", 'buddyboss-marketplace' ), $plugin_link );
