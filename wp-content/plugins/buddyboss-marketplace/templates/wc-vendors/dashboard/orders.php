...
jQuery(function () {
    jQuery('a.view-items').on('click', function (e) {
        ...
        if ( jQuery(this).text() == "<?php _e('Hide items', 'buddyboss-marketplace'); ?>" ) {
        	jQuery(this).text("<?php _e('View items', 'buddyboss-marketplace'); ?>");
    	} else {
        	jQuery(this).text("<?php _e('Hide items', 'buddyboss-marketplace'); ?>");
    	}
...
<h2 class="orders-title"><?php _e( 'Orders', 'buddyboss-marketplace' ); ?></h2>

<?php global $woocommerce; ?>

<?php if ( function_exists( 'wc_print_notices' ) ) { wc_print_notices(); } ?>

<table class="table table-condensed table-vendor-sales-report orders">
	<thead>
	<tr>
	<th class="product-header"><?php _e( 'Order', 'buddyboss-marketplace' ); ?></th>
	<th class="quantity-header"><?php _e( 'Shipping', 'buddyboss-marketplace' ) ?></th>
	<th class="commission-header"><?php _e( 'Total', 'buddyboss-marketplace' ) ?></th>
	<th class="rate-header"><?php _e( 'Date', 'buddyboss-marketplace' ) ?></th>
	<th class="rate-header"><?php _e( 'Links', 'buddyboss-marketplace' ) ?></th>
	</thead>
	<tbody>
		...
		<?php foreach ( $order_summary as $order ) :
			...
			<tr id="order-<?php echo $order->id; ?>" data-order-id="<?php echo $order->id; ?>">
				...
				$order_actions = array(
					'view'		=> array(
						'class' 	=> 'view-items',
						'content'	=> __('View items', 'buddyboss-marketplace'),
					)
				);
				if ( $needs_shipping ) {
					$order_actions['shipped'] = array(
						'class' 	=> 'mark-shipped',
						'content'	=> __('Mark shipped', 'buddyboss-marketplace'),
				...
				if ( $shipped ) {    
					$order_actions[ 'shipped' ] = array(
					'class' 	=> 'mark-shipped',
					'content'	=> __('Shipped', 'buddyboss-marketplace'),
					'url'		=> '#'
					);
				} 
				...
				if ( $providers && $needs_shipping && class_exists( 'WC_Shipment_Tracking' ) ) {
					$order_actions['tracking'] = array(
						...
						'content'	=> __( 'Tracking', 'buddyboss-marketplace' )
					);
				}
	...
	<?php else : ?>
		...
		<tr>
			<td colspan="5" class="empty"><?php _e( 'You have no orders during this period.', 'buddyboss-marketplace' ); ?></td>
		</tr>
