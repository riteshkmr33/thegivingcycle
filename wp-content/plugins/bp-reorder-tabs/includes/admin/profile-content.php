        ...
        <tr>
            ...
            <td>
                ...
                <select name="def_profile_page" id="def_profile_page">
                    ...
                    <?php
                    foreach( $bp_navs as $nav ){
                        if( in_array( $nav->slug, $private_navs ) )
                            continue;

                        $selected = $nav->slug==$default_nav ? ' selected' : '';
                        ...
                        echo "<option value='" . esc_attr( $nav->slug ) . "' {$selected}>{$nav_name}</option>";
                    }
                    ?>
                </select>
            </td>
    ...
    <ul class="nav-list sortable">
        ...
        if( !empty( $reordered_group_navs ) ){
            ...
            foreach( $reordered_group_navs as $nav ){
                ...
                if ( ! empty( $nav_props['name'] ) && 'Activity' === $nav_props['name'] &&  ! empty( $activity_label ) ) {
                    $nav_name = $activity_label;
                }

                $subnavs = $nav_props['subnavs'];
                ...
                if( !empty( $subnavs ) ) {
                    ...
                    foreach( $subnavs as $subnav_key =>$subnav ){

                        //make sure to convert it into array
                        if( is_object( $subnav ) ){
                            $subnav = get_object_vars( $subnav );
                        }
                        ...
                        echo "<li class='nav nav-subnav nav-type-profile nav-" . esc_attr( $subnav_key ) ."' data-navid='" . esc_attr( $subnav_key ) . "'>";
                        echo "<div class='drag-handle'>{$subnav['name']}</div>";
                        echo "</li>";
                    }
