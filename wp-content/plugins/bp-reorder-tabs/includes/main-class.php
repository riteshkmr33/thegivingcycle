...
class BuddyPress_Reorder_Tabs_Plugin {
	...
	public static function instance(){
		...
		if ( null === $instance )
		{
	         ...
	         */
	        if ( version_compare( buddypress()->version, '2.6', '<' ) ){
	...
	public function bp_loaded(){
		add_action( 'bp_screens',				array( $this, 'change_profile_tab_order' ), 101 );
		/**
		...
		add_action( 'bp_screens',                  array( $this, 'change_subnavs' ), 101 );
        ...
        // nouveau...
		add_filter( 'bp_after_nouveau_appearance_settings_parse_args', array( $this, 'unset_nouveau_nav_order' ), 10, 1 );
	}
	...
	public function change_subnavs(){
		...
		if( isset( $profile_settings['config_data'] ) && !empty( $profile_settings['config_data'] ) ){
			...
			foreach( $topnavs as $topnav=>$topnav_settings ){
                $topnav_settings = (array) $topnav_settings;
				...
				if( isset( $topnav_settings['subnavs'] ) && !empty( $topnav_settings['subnavs'] ) ){
					...
					foreach( $subnavs as $subnav=>$subnav_props ){
                        ...
                        $subnav_props = (array) $subnav_props;
	...
	public function save_profile_navs_info(){
		...
		foreach( $bp->members->nav->get_primary() as $nav_outer ){
			...
			if( ( $subnavs = $bp->members->nav->get_secondary( array( 'parent_slug' => $nav_outer->slug ) ) ) != '' ){
				foreach( $subnavs as $nav_inner ){
					$item_inner[$nav_inner->slug] = array(
						'name'		=> $nav_inner->name,
		...
		if( !empty( $config_data ) ){
			foreach( $config_data as $s_nav=>$s_nav_props ){
				...
				/** Override sub navigation menu position */
                $s_subnav_props = (array)$s_nav_props['subnavs'];
				foreach ( $s_subnav_props as $s_subnav_slug => $s_subnav ) {
                    $s_subnav = (array)$s_subnav;
                    if( isset( $s_subnav['position'] ) ){
                        if( isset( $profile_navs[$s_nav]['subnavs'][$s_subnav_slug] ) )
                            $profile_navs[$s_nav]['subnavs'][$s_subnav_slug]['position'] = $s_subnav['position'];
                    }
                }
			}
		}
	...
	public function reorder_wp_menus() {
		...
		if ( isset( $profile_settings[ 'config_data' ] ) && ! empty( $profile_settings[ 'config_data' ] ) ) {
            ...
            foreach ( $bp->active_components as $key => $value ) {
                ...
                if ( empty( $bp->{$key}->admin_menu ) ) continue;

                foreach ( $bp->{$key}->admin_menu as $admin_nav_item ) {
                    $admin_items_arr[] = array(
			...
			foreach ( $config as $nav => $nav_settings ) {
				...
				if ( 'profile' == $nav ) {
					...
					/** Sub navigation menu nodes */
                    if( isset( $nav_settings['subnavs'] ) && !empty( $nav_settings['subnavs'] ) ) {
                        $subnavs = (array)$nav_settings['subnavs'];

                        foreach ($subnavs as $subnav => $subnav_props) {
                            if ( $subnav_node = $wp_admin_bar->get_node( 'my-account-x' . $nav .'-'. $subnav) ) {
                                $nav_data_arr[ $nav .'-'. $subnav ] = $subnav_node;
                                $wp_admin_bar->remove_node( 'my-account-x' . $nav .'-'. $subnav );
                            }
                        }
                    }
                            /**
                    ...
                    if ( ! empty( $nav_data ) ) {
                        ...
                        if ( isset( $nav_settings['subnavs'] ) && !empty( $nav_settings['subnavs'] ) ) {
                            ...
                            foreach ($subnavs as $subnav => $subnav_props) {
                                ...
                                if ( $subnav_node = $wp_admin_bar->get_node( 'my-account-' . $nav .'-'. $subnav) ) {
                                    $nav_data_arr[ $nav .'-'. $subnav ] = $subnav_node;
                                    $wp_admin_bar->remove_node( 'my-account-' . $nav .'-'. $subnav );
                                }
                            }
                        }
                        ...
                        /* Check if menu exists and then continue */
                        if( $bp_navs ){
                        ...
                        if( !empty( $bp_nav_item_link )){
                            ...
                            if( isset( $nav_settings['subnavs'] ) && !empty( $nav_settings['subnavs'] ) ) {
                                ...
                                foreach ($subnavs as $subnav => $subnav_props) {
                                    ...
                                    if ( $subnav_node = $wp_admin_bar->get_node( $wp_admin_nav_item .'-'. $subnav) ) {
                                        $nav_data_arr[ $nav .'-'. $subnav ] = $subnav_node;
                                        $wp_admin_bar->remove_node( $wp_admin_nav_item .'-'. $subnav );
                                    }
                                }
                            }
                        }
			...
			foreach ( $config as $nav => $nav_settings ) {
                ...
                if( isset( $nav_settings['subnavs'] ) && !empty( $nav_settings['subnavs'] ) ) {
                    ...
                    foreach ($subnavs as $subnav => $subnav_props) {
                        ...
                        if ( isset( $nav_data_arr[$nav .'-'. $subnav] ) ) {
                            $wp_admin_bar->add_menu( array(
                                'parent' => $nav_data_arr[$nav .'-'. $subnav]->parent,
                                'id' => $nav_data_arr[$nav .'-'. $subnav]->id,
                                'title' => $nav_data_arr[$nav .'-'. $subnav]->title,
                                'href' => $nav_data_arr[$nav .'-'. $subnav]->href
                            ) );
                        }
                    }
                }
	...
	/**
	 * Unset the user nav order set from BuddyPress theme customizer option
	 *
	 * @param $r
	 * @return mixed
	 */
	public function unset_nouveau_nav_order( $r ) {
		unset( $r['user_nav_order'] );
		unset( $r['group_nav_order'] );
		return $r;
	}
