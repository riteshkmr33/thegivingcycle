...
if (!defined( 'BUDDYBOSS_REORDER_TABS_PLUGIN_VERSION' ) ) {
  define( 'BUDDYBOSS_REORDER_TABS_PLUGIN_VERSION', '2.0.1' );
}
...
function buddypress_reorder_tabs_init()
{
  global $bp, $BUDDYBOSS_REORDER_TABS;
    ...
    if ( ! function_exists('bp_is_active' ) ) {
        add_action('admin_notices','buddypress_reorder_tabs_install_notice');
        return;
    }
 ...
 */
function buddypress_reorder_tabs_install_notice() {
    echo '<div id="message" class="error fade"><p style="line-height: 150%">';
    _e( sprintf( '<strong>BuddyPress Reorder Tabs</strong> requires the BuddyPress plugin to work. Please install <a href="%s" target="_blank">BuddyPress</a> first.', 'https://buddypress.org/' ), 'bp-reorder-tabs');
    echo '</p></div>';
}
