...
class WCVendors_Pro_Order_Controller {
	...
	public function __construct( $wcvendors_pro, $version, $debug ) {
		...
		$this->start_date 	= ( !empty( $_SESSION[ 'PV_Session' ][ '_wcv_order_start_date_input' ] ) ) 	? $_SESSION[ 'PV_Session' ][ '_wcv_order_start_date_input' ] : strtotime( apply_filters( 'wcv_order_start_date', $this->default_start ) );
			...
			foreach ( $all_orders as $_order ) {
				...
				if ( !empty( $_order->order_items ) ){
					...
					foreach ( $_order->order_items as $item ) {
						...
						$products_html .=  $_product->get_sku() ? sprintf( __( 'SKU: %s %s', 'wcvendors-pro' ), $_product->get_sku(), '<br />' ) : '';
						$products_html .= do_action('woocommerce_order_item_meta_start', $item->get_id(), $item, $order);
						$products_html .= wc_display_item_meta( $item, ['echo' => false ] );
						$products_html .= do_action('woocommerce_order_item_meta_end', $item->get_id(), $item, $order);
				...
				$hide_view_details 		= 'yes' === get_option( 'wcvendors_hide_order_view_details' ) ? true : false;
				$hide_shipping_label 	= 'yes' === get_option( 'wcvendors_hide_order_shipping_label' ) ? true : false;
				$hide_order_note 		= 'yes' === get_option( 'wcvendors_hide_order_order_note' ) ? true : false;
				$hide_tracking_number	= 'yes' === get_option( 'wcvendors_hide_order_tracking_number' ) ? true : false;
				$hide_mark_shipped		= 'yes' === get_option( 'wcvendors_hide_order_mark_shipped' ) ? true : false;
				...
				$hide_phone 		= get_option( 'wcvendors_capability_order_customer_phone' );
	...
	public function table_actions() {
		...
		$can_export_csv = 'yes' === get_option( 'wcvendors_capability_orders_export' ) ? true : false;
	...
	public function existing_order_notes( $order_id ) {
		...
		$can_view_comments = get_option( 'wcvendors_can_view_order_comments' );
   ...
   /**
	...
	public function export_csv() {
		...
		$date_range = array(
			...
			'after'  => date( 'Y-m-d', ( !empty( $_SESSION[ 'PV_Session' ][ '_wcv_order_start_date_input' ] ) ) ? $_SESSION[ 'PV_Session' ][ '_wcv_order_start_date_input' ] : strtotime( apply_filters( 'wcv_order_start_date', $this->default_start ) )),
		);
	...
	public function get_vendor_tracking_details( $order_id, $vendor_id ) {
		...
		if ( empty( $order_tracking_details ) ) return array();
	...
	public function filter_formatted_shipping_address( $address ){
		...
		$hide_name 				= get_option( 'wcvendors_capability_order_customer_name' );
		$hide_shipping_address 	= get_option( 'wcvendors_capability_order_customer_shipping' );
	...
	public function filter_formatted_billing_address( $address ){
		...
		$hide_name 				= get_option( 'wcvendors_capability_order_customer_name' );
		$hide_billing_address 	= get_option( 'wcvendors_capability_order_customer_shipping' );
	...
	*/
	public function filter_order_item_get_formatted_meta_data( $formatted_meta, $order_item ){
		...
		$hide_meta = apply_filters( 'wcvendors_hide_order_meta_data', array(
			'Sold By',
