...
class WCVendors_Pro_Vendor_Controller {
	...
	public static function save_pending_vendor( $vendor_id ) {
		...
		$manual = get_option( 'wcvendors_manual_vendor_registration' );
	...
	public function process_submit() {
		...
		// SEO
		update_user_meta( $vendor_id, 'wcv_seo_title', 					sanitize_text_field( $_POST['wcv_seo_title'] ) );
		update_user_meta( $vendor_id, 'wcv_seo_meta_description', 		sanitize_text_field( $_POST['wcv_seo_meta_description'] ) );
		update_user_meta( $vendor_id, 'wcv_seo_meta_keywords', 			sanitize_text_field( $_POST['wcv_seo_meta_keywords'] ) );
		update_user_meta( $vendor_id, 'wcv_seo_fb_title', 				sanitize_text_field( $_POST['wcv_seo_fb_title'] ) );
		update_user_meta( $vendor_id, 'wcv_seo_fb_description', 		sanitize_text_field( $_POST['wcv_seo_fb_description'] ) );
		update_user_meta( $vendor_id, 'wcv_seo_fb_image_id', 			sanitize_text_field( $_POST['wcv_seo_fb_image_id'] ) );
		update_user_meta( $vendor_id, 'wcv_seo_twitter_title', 			sanitize_text_field( $_POST['wcv_seo_twitter_title'] ) );
		update_user_meta( $vendor_id, 'wcv_seo_twitter_description', 	sanitize_text_field( $_POST['wcv_seo_twitter_description'] ) );
		update_user_meta( $vendor_id, 'wcv_seo_twitter_image_id', 		sanitize_text_field( $_POST['wcv_seo_twitter_image_id'] ) );
	...
	public function enable_vendor_tools( $product_id ) {
		...
		if ( get_current_user_id() == $post->post_author && WCV_Vendors::is_vendor( get_current_user_id() ) ) {
			...
			$disable_delete 		= get_option( 'wcvendors_delete_product_cap');
			$disable_duplicate 		= get_option( 'wcvendors_duplicate_product_cap');
			...
			$actions = apply_filters( 'wcv_product_single_actions' , array(
				'edit'  	=>
						apply_filters( 'wcv_product_single_actions_edit', array(
							...
							'class'		=> '',
							'url' 		=> WCVendors_Pro_Dashboard::get_dashboard_page_url( 'product/edit/' . $product_id  ),
						) ),
				'duplicate'  	=>
	...
	*/
	public function json_unique_store_name(){
		...
		// Check if the Shop name is unique
		$users = get_users( array( 'meta_key' => 'pv_shop_slug', 'meta_value' => sanitize_title( $store_name ) ) );
