...
jQuery( function( $ ){
	...
	function show_and_hide_panels() {
		...
		$( hide_classes ).find( 'input' ).removeClass( 'is_hidden' );
		$( show_classes ).hide().addClass( 'is_hidden' );
		$( show_classes ).find( 'input' ).addClass( 'is_hidden' );

		// Shows rules
		if ( is_downloadable ) {
			$('.show_if_downloadable').show().removeClass( 'is_hidden' );
			$('.show_if_downloadable').find( 'input' ).removeClass( 'is_hidden' );
		}
		if ( is_virtual ) {
			$('.show_if_virtual').show().removeClass( 'is_hidden' );
			$('.show_if_virtual').find( 'input' ).removeClass( 'is_hidden' );
		}

        $('.show_if_' + product_type).show().removeClass( 'is_hidden' );
        $('.show_if_' + product_type).find( 'input' ).removeClass( 'is_hidden' );

		// Hide rules
		if ( is_downloadable ) {
			$('.hide_if_downloadable').hide().addClass( 'is_hidden' );
			$('.hide_if_downloadable').find( 'input' ).addClass( 'is_hidden' );
		}
		if ( is_virtual ) {
			$('.hide_if_virtual').hide().addClass( 'is_hidden' );
			$('.hide_if_virtual').find( 'input' ).addClass( 'is_hidden' );
		}
		...
		$('.hide_if_' + product_type).find('input').addClass( 'is_hidden' );
		$('input#_manage_stock').change();

		// reset form validation for fields that have been hidden.
		$( '.wcv-form' ).parsley().reset();
	...
	$('input#_manage_stock').change(function(){
		if ( $(this).is(':checked') ) {
			...
			$('div.stock_fields').find( 'input' ).removeClass( 'is_hidden' );
			$( '.wcv-form' ).parsley().reset();
		} else {
			$('div.stock_fields').hide().addClass( 'is_hidden' );
			$('div.stock_fields').find( 'input' ).addClass( 'is_hidden' );
			$( '.wcv-form' ).parsley().reset();
		}
	}).change();
	...
	*/
	...
	$( '#wcv-product-edit' ).submit( function( e ) {
		...
		// Populate the file_url fields that are hidden.THis stops vendors from manipulating urls if this is forced to file name display only.		
		if ( ! $( '.downloadable_files' ).parent().hasClass( 'is_hidden' ) ){
			...
			if ( file_count > 0 ){
				$( '.download_file').each( function( index ) {
				 	if ( $( this ).find( 'td.file_name' ).find( 'input' ).val() !== '' ){
				 		$( this ).find( '.file_url' ).val( $( this ).find( '.file_display' ).val() );
				 	}
				});
			}
		}
		...
		if ( wcv_frontend_product.require_category == 1 ){
			...
			if ( $( '.wcv_category_check' ).length ) {
				...
				if ( category_count <= 0 ) {
		        	...
		        	$( '.wcv_terms_checklist_container').css( {
		        		 'color': '#b94a48',
