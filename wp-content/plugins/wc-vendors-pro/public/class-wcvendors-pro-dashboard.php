...
class WCVendors_Pro_Dashboard {
	...
	public function load_page( $object = 'dashboard', $action = '', $object_id = null, $template = null, $custom = null )
	{
		...
		// Vendor Store Notice
		$vendor_dashboard_notice = get_option( 'wcvendors_vendor_dashboard_notice' );
	...
	public function get_dashboard_quick_links(){
		...
		$products_disabled		= 'yes' === get_option( 'wcvendors_product_management_cap' ) ? true : false;
		$coupons_disabled		= 'yes' === get_option( 'wcvendors_shop_coupon_management_cap' ) ? true : false;
		...
		if ( ! wc_coupons_enabled() ) $coupons_disabled = true;
	...
	public function get_dashboard_pages()  {
		...
		$disable_duplicate 		= 'yes' === get_option( 'wcvendors_capability_product_delete' ) ? true : false;
	...
	public function can_view_dashboard() {
		...
		if ( !is_user_logged_in() ) {
			...
			if ( WCVendors_Pro_Vendor_Controller::is_pending_vendor( get_current_user_id() ) ) {
				...
				wc_get_template( 'vendor-signup-form.php', array( 'vendor_signup_notice' => $vendor_signup_notice, 'shipping_details' => array() ), 'wc-vendors/front/', $this->base_dir . '/templates/front/' );
				return false;
			} else {
	...
	*/
	public function create_nav( ) {
		...
		$products_disabled		= 'yes' === get_option( 'wcvendors_product_management_cap' ) ? true : false;
		$orders_disabled		= 'yes' === get_option( 'wcvendors_order_management_cap' ) ? true : false;
		$coupons_disabled		= 'yes' === get_option( 'wcvendors_shop_coupon_management_cap' ) ? true : false;
		$ratings_disabled		= 'yes' === get_option( 'wcvendors_ratings_management_cap' ) ? true : false;
		$settings_disabled		= 'yes' === get_option( 'wcvendors_settings_management_cap' ) ? true : false;
		$viewstore_disabled		= 'yes' === get_option( 'wcvendors_view_store_cap' ) ? true : false;
		...
		$menu_wrapper_start = apply_filters( 'wcv_dashboard_nav_wrapper_start', '<div class="wcv-cols-group wcv-horizontal-gutters"><div class="all-100"><nav class="wcv-navigation ' . $nav_class . ' "><ul class="menu horizontal black flyout">' );
	...
	*/
	public function page_disabled(){
		...
		switch ( $current_page ) {
			case 'product':
				return 'yes' !== get_option( 'wcvendors_product_management_cap' ) ? true : false;
				break;
			case 'order':
				return 'yes' !== get_option( 'wcvendors_order_management_cap' ) ? true : false;
				break;
			case 'shop_coupon':
				return 'yes' !== get_option( 'wcvendors_shop_coupon_management_cap' ) ? true : false;
				break;
			case 'rating':
				return 'yes' !== get_option( 'wcvendors_ratings_management_cap' ) ? true : false;
				break;
			case 'settings':
				return 'yes' !== get_option( 'wcvendors_settings_management_cap' ) ? true : false;
				break;
			default:
				return true;
				break;
