...
class WCVendors_Pro_Shop_Coupon_Controller {
	...
	public function process_submit() {
		...
		if ( ( isset( $_POST[ '_wcv_coupon_post_meta_apply_to_all_products' ] ) && 'yes' == $_POST[ '_wcv_coupon_post_meta_apply_to_all_products' ] ) ){
			$product_ids = array_filter( array_map( 'intval', explode( ',', $all_vendor_product_ids ) ) );
			update_post_meta( $coupon, 'apply_to_all_products', $_POST[ '_wcv_coupon_post_meta_apply_to_all_products' ] );
		} else {
			$product_ids = isset( $_POST['_wcv_coupon_post_meta_product_ids'] ) ? array_filter( array_map( 'intval', (array) $_POST['_wcv_coupon_post_meta_product_ids'] ) ) : array();
		}

		// Free shipping
		if ( isset( $_POST[ '_wcv_coupon_post_meta_free_shipping' ] )  && 'yes' == $_POST[ '_wcv_coupon_post_meta_free_shipping' ] ){
			update_post_meta( $coupon, 'vendor_free_shipping', $_POST[ '_wcv_coupon_post_meta_apply_to_all_products' ] );
		} else {
			delete_post_meta( $coupon, 'vendor_free_shipping' );
		}

		$coupon_object = new WC_Coupon( $coupon );
		$coupon_object->set_props( array(
			...
			'excluded_product_ids'        => isset( $_POST['_wcv_coupon_post_meta_exclude_product_ids'] ) ? array_filter( array_map( 'intval', (array) $_POST['_wcv_coupon_post_meta_exclude_product_ids'] ) ) : array(),
	...
	public function process_delete( ) {
		...
		if ( isset( $wp->query_vars[ 'object' ] ) ) {
			...
			if ( $object == 'shop_coupon' && $action == 'delete' && is_numeric( $id ) ) {
				...
				if ( 'yes' !== get_option( 'wcvendors_vendor_coupon_trash' ) ) {
	...
	public function table_posts_per_page( $per_page ) {
		...
		return get_option( 'wcvendors_coupons_per_page' );
