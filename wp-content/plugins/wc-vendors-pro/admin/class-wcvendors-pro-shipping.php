...
class WCVendors_Pro_Shipping_Method extends WC_Shipping_Method {
	...
	public function calculate_shipping( $package = array() ) {
			...
			foreach ( $package['contents'] as $item_id => $cart_item ) {
				...
				if ( $cart_item['quantity'] > 0 ) {
					...
					if ( $cart_item['data']->needs_shipping() ) {
						...
						if ( self::$debug ) WCVendors_Pro::log( $rate, 'RATE' );

						if ( $rate ) {

							$qty = ( $rate->qty_override === 'yes' && empty( $rate->max_charge_product ) ) ? 1 : (int) $cart_item[ 'quantity' ];

							if ( self::$debug ) WCVendors_Pro::log( $qty, 'QTY' );
							...
							$product_fee 		= $this->get_fee( (int) $rate->product_fee, (int) $item_shipping_cost ) * (int) $qty;
							...
							if ( $this->tax_status === 'taxable' && wc_tax_enabled() && $item_shipping_cost > 0 ) {
								...
								if ( self::$debug ) WCVendors_Pro::log( $tax_rates );
		...
		if ( self::$debug ) WCVendors_Pro::log( $settings, 'SETTINGS' );
		if ( self::$debug ) WCVendors_Pro::log( $order_spend_total, 'Order Spend' );
		...
		} else if ( $settings[ 'min_charge' ] > 0 && $shipping_costs < $settings[ 'min_charge' ] ) {
		...
		} else if ( $settings[ 'max_charge' ] > 0 && $shipping_costs > $settings[ 'max_charge' ] ) {
		...
		if ( self::$debug ) WCVendors_Pro::log( $taxes );
	...
	public static function get_shipping_rate( $product_id, $vendor_id, $package, $settings ) {
		...
		if ( self::$debug ) WCVendors_Pro::log( $product_rates, 'PRODUCT RATES');
		if ( self::$debug ) WCVendors_Pro::log( $store_rates, 'STORE RATES' );
		...
		if ( $shipping_type == 'flat' ) {
			...
			if ( $customer_country == $store_country ) {
				...
				if ( ( is_array( $product_rates ) && array_key_exists( 'national_disable', $product_rates ) && 'yes' === $product_rates[ 'national_disable'] ) ) {
					...
					if ( self::$debug )  WCVendors_Pro::log( 'product national disabled' );
					...
					if ( self::$debug ) WCVendors_Pro::log( 'product national triggered' );
					...
					if ( self::$debug )  WCVendors_Pro::log( 'store rate national disabled' );
					...
					if ( self::$debug )  WCVendors_Pro::log( 'store rate international triggered' );
					...
					if ( self::$debug )  WCVendors_Pro::log( 'global settings triggered' );
					...
					if ( self::$debug )  WCVendors_Pro::log( 'no national flat rate found' );
				...
				if ( ( is_array( $product_rates ) && array_key_exists( 'international_disable', $product_rates ) && 'yes' === $product_rates['international_disable'] ) ) {
					...
					if ( self::$debug ) WCVendors_Pro::log( 'international disabled' );
					...
					if ( self::$debug ) WCVendors_Pro::log( 'international product rate' );
					...
					if ( self::$debug ) WCVendors_Pro::log( 'store rate international disabled' );
					...
					if ( self::$debug ) WCVendors_Pro::log( 'international store rate' );
	...
	public static function check_postcode( $customer_postcode, $rate_postcode ){
		...
		if ( $range_position > 0 ){
			...
			if ( self::$debug ) WCVendors_Pro::log( $min_postcode, 'Min Post Code' );
			if ( self::$debug ) WCVendors_Pro::log( $max_postcode, 'Max Post Code' );

			$customer_postcode = ( is_numeric( $customer_postcode ) ) ? $customer_postcode : self::make_numeric_postcode( $customer_postcode );

			if ( self::$debug ) WCVendors_Pro::log( $customer_postcode, 'Customer Post Code' );
			...
			if ( self::$debug ) WCVendors_Pro::log( $in_range, 'In Range' );
