...
class WC_Software_License_Client {
	...
	*/
	...
	public function license_inactive(){
		...
		if ( ! in_array( $screen->id, wcv_get_screen_ids() ) && 'plugins' !== $screen->id ) return;
	...
	*/
	public function license_expired(){
		...
		echo '<div class="error notice is-dismissible"><p>'.
			sprintf( __( '<strong>The %s license key has expired, you will be unable to get automatic updates or support! </strong> %sClick here%s to purchase your new support and updates license key.', 'wcvendors-pro' ), $this->plugin_nice_name, '<a href="' . $this->renewal_url . '">', '</a>' ) .
		'</p></div>';
	}
