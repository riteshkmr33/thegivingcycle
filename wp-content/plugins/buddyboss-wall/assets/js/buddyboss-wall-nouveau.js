...
window.Code.BuddyBoss_Wall = ( function( window, $, util, undefined ) {
	...
	var Wall = {
		...
		success: function( target, type, is_a_comment, response, text, xhr ) {
			...
			if ( responseJSON && responseJSON.hasOwnProperty( 'fav_count' ) ) {

				// If the count is above 0
				if ( responseJSON.fav_count ) {
					has_like_text = true;
			...
			target.fadeOut( 200, function() {
				...
				if( is_a_comment ){
					comments_wrap	= button.parents('.activity-meta');
					comments_ul		= comments_wrap.find('ul').first();
          ...
          comments_ul: comments_ul
				...
				if ( is_a_comment ) {
					...
					button.find('.likes-count').text( responseJSON.fav_count );
				}
				//button.attr('title', 'fav' == type ? state.remove_fav : state.mark_as_fav);
				button.fadeIn(200);
