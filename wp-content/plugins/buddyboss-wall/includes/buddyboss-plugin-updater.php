...
add_filter( 'bboss_licensed_packages', 'bbwall_register_licensed_package' );
function bbwall_register_licensed_package( $packages=array() ){
    $this_package = array(
        'id'        => 'bbwall',
        'name'      => __( 'BuddyBoss Wall', 'buddyboss-wall' ),
        'products'  => array(
    ...
    $packages['bbwall'] = $this_package;
    return $packages;
}

add_filter( 'bboss_updatable_products', 'bbwall_register_updatable_product' );
function bbwall_register_updatable_product( $products ){
    ...
    $products['BBWALL'] = array(
        ...
        'path'      => 'buddyboss-wall/buddyboss-wall.php',
        'id'        => 37,
        'software_ids'  => array( 'BBWALL_1S', 'BBWALL_5S', 'BBWALL_20S' ),
        'type'      => 'plugin',
    );
...
if( !function_exists( 'bboss_notice_install_updater' ) ) {
    ...
    function bboss_notice_install_updater(){
        ...
        $notice_install_buddyboss_updater = sprintf( __( "To receive updates for BuddyBoss products, please install and activate the plugin %s", 'buddyboss-wall' ), $plugin_link );
